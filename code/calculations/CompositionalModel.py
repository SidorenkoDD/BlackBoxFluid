...
class CompositionalModel:
    ...
    def __init__(self, zi: Composition, p, t):
        self.composition = zi
        self.conditions = Conditions(p, t)

        self.phase_stability = PhaseStability(self.composition, self.conditions.p, self.conditions.t)
        ...
        # Если система нестабильна, то передаем К из анализа стабильности и запускаем расчет flash
            ...
            if (self.phase_stability.S_l > 1) and (self.phase_stability.S_v > 1):
                if self.phase_stability.S_l > self.phase_stability.S_v:
                    self.phase_equilibrium = PhaseEquilibrium(self.composition, self.conditions.p,
                                                               self.conditions.t, self.phase_stability.k_values_liquid)
                else:
                    self.phase_equilibrium = PhaseEquilibrium(self.composition, self.conditions.p,
                                                               self.conditions.t, self.phase_stability.k_values_vapour )
            ...
            self.fluid_properties = FluidProperties(self.conditions.p, self.conditions.t, equil_obj= self.phase_equilibrium)
    ...
    def show_results(self):
        ...
        print('=====')
        print(f'FLASH RESULTS: P {self.conditions.p}, T: {self.conditions.t}')
...
if __name__ == '__main__':
    ...
    comp_model = CompositionalModel(comp, 6, 50)
